...
uint8_t ST7565_vert = 1;
uint8_t ST7565_private_font = 0;

unsigned short pos;
unsigned char width, height;

uint8_t font_color = 1;
...
bounding_box_t draw_bar(unsigned char x, unsigned char y, unsigned char w, unsigned char h, unsigned char color){
	bounding_box_t ret;
	ret.x1 = x;
	ret.y1 = y;
	ret.x2 = x;
  ...
  for (j=0; j<h; j++) {
    for (i=0; i<w; i++ ) {
			if(ST7565_vert)
					glcd_pixel(x + j, 64-(y + i), color);
			else{
					glcd_pixel(x + i, (y + j), color);
			}
		}
	}
	if(ST7565_vert){
...
bounding_box_t draw_char(unsigned char c, unsigned char x, unsigned char y, unsigned char font[]) {
	...
	if(!ST7565_private_font){
		...
		width = font[pos];
		height = font[FONT_HEADER_HEIGHT];
	...
	for (i = 0; i < width; i++) {
		...
		for (unsigned char j = 0; j < height; j++) {
			...
			if(ST7565_vert)
				if (font[pos] & 1 << (j % 8)) {
					glcd_pixel(x + j, 64-(y + i), font_color);
				} else {
					glcd_pixel(x + j, 64-(y + i), !font_color);
				}
			else{
				if (font[pos] & 1 << (j % 8)) {
					glcd_pixel(x + i, (y + j), font_color);
				} else {
					glcd_pixel(x + i, (y + j), !font_color);
				}
			}
...
unsigned char text_width(char *string, unsigned char font[], unsigned char spacing) {

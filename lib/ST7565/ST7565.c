...
void  ST7565_drawchar_vert(uint8_t x, uint8_t line, char c) {
  uint8_t i;
  for (i =0; i<7; i++ ) {
    //st7565_buffer[x + (line*128) ] = font_1[((unsigned char)c * 7) + i];
		//st7565_buffer[x + (line*128) ] = font_1[i+7]; 
		//st7565_buffer[x + (line*128) ] = font_1[i+7];  
    x++;
  }

  ST7565_updateBoundingBox(x-7, line*6, x-1, line*6 + 6); 
}
...
//    digitalWrite(cs, LOW);
  ...
  // ADC select
  ST7565_st7565_command(CMD_SET_ADC_NORMAL);//HORIZANTLY
	//ST7565_st7565_command(CMD_SET_ADC_REVERSE);
  // SHL select
  //ST7565_st7565_command(CMD_SET_COM_NORMAL);
	ST7565_st7565_command(CMD_SET_COM_REVERSE);//VERTICAL
...
inline void ST7565_spiwrite(uint8_t c) {
  ...
  //while (!__HAL_SPI_GET_FLAG(hspi2_, SPI_FLAG_TXE));
	//while(HAL_SPI_GetState(hspi2_)!=HAL_OK){};
  /*HAL_SPI_Transmit(hspi2_, &c, sizeof(c), 10);
	while(HAL_SPI_GetState(hspi2_) != HAL_SPI_STATE_READY){};*/
		
  
    int8_t i;
    for (i=7; i>=0; i--) {
        //HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_RESET);//SCK
        //DWT_Delay_us(1);
        digitalWrite(SPI_SCK_PIN1, 0);
        ets_delay_us(1);
        
        if (c & _BV(i))
            //HAL_GPIO_WritePin(GPIOC, GPIO_PIN_7, GPIO_PIN_SET);//data
          digitalWrite(SPI_MOSI_PIN1, 1);
        else {
            //HAL_GPIO_WritePin(GPIOC, GPIO_PIN_7, GPIO_PIN_RESET);//data
          digitalWrite(SPI_MOSI_PIN1, 0);  }
        ets_delay_us(1);
				//DWT_Delay_us(1);
        //HAL_GPIO_WritePin(GPIOC, GPIO_PIN_6, GPIO_PIN_SET);//SCK
        digitalWrite(SPI_SCK_PIN1, 1);
    }	
}
